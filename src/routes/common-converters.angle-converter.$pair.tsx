...
import { angleArticle } from "@/data/angleArticles";
...
function buildPair(slug: string): PairData {
  const parsed = parseAnglePairSlug(slug);
  if (!parsed) throw notFound();
  const from = angleUnitById.get(parsed.from)!;
  const to = angleUnitById.get(parsed.to)!;
  const ratio = from.factor / to.factor;
  const shortTitle = `${angleShort(from)} to ${angleShort(to)}`;
...
export const Route = createFileRoute("/common-converters/angle-converter/$pair")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => {
    ...
    const path = `/common-converters/angle-converter/${loaderData.slug}`;
    return {
      meta: [
  ...
  return (
    <main className="mx-auto max-w-md px-4 pb-16">
      ...
      <div className="mt-2 flex items-center justify-end gap-2">
        <button
          type="button"
          aria-label={`Swap to ${angleShort(to)} to ${angleShort(from)}`}
          title={`Swap to ${angleShort(to)} to ${angleShort(from)}`}
          onClick={() =>
            navigate({
            ...
            {pairTableAmounts.map((n) => (
              <tr key={n}>
                ...
                <td className="border-b border-line py-1.5 text-right break-all">
                  {formatResult(convertAngle(n, from.id, to.id))} {angleShort(to)}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <section className="mt-7">
        ...
        <div className="grid grid-cols-2 gap-x-3 border-t border-line">
          ...
          >
            {angleShort(to)} to {angleShort(from)}
          </Link>
          {related.map(([f, t]) => (
            <Link
              key={`${f}-${t}`}
            ...
            >
              {angleShort(angleUnitById.get(f)!)} to {angleShort(angleUnitById.get(t)!)}
            </Link>
          ))}
        </div>
        <Link
      ...
      <section className="mt-7">
        <h2 className="mb-2 text-[12px] tracking-[0.12em] uppercase">
          Convert {unitTitle(from)} to all angle units
        </h2>
        <table className="w-full table-fixed border-t border-line text-[12px]">
          <thead>
            <tr className="text-[10px] tracking-[0.14em] text-mute uppercase">
          ...
          <tbody>
            {allTargets.map((u) => (
              <tr key={u.id}>
                ...
                <td className="border-b border-line py-1.5 text-right break-all">
                  {formatResult(convertAngle(1, from.id, u.id))} {angleShort(u)}
                </td>
              </tr>
            ))}
          </tbody>
